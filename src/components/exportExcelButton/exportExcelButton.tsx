...
export function ExportExcelButton({ companies, selectedFields }: ExportExcelButtonProps) {
    const exportToExcel = () => {
      const data = companies.map((company) => {
        const rowData: { [key: string]: string | undefined } = {}; // Alterado para permitir valores undefined
        selectedFields.forEach((field) => {
          switch (field) {
            ...
            case 'contactEmail':
              ...
              break;
            case 'status':
              rowData['Status'] = company.status || ''; // Atribui um valor padrão para evitar undefined
              break;
